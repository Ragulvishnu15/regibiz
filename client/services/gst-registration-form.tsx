// services/gst-registration-form.tsx
  ...
  return (
    <div className="bg-gradient-to-r from-emerald-900/30 to-emerald-800/10 border border-emerald-500/20 rounded-xl p-4 md:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-lg mb-8 relative overflow-hidden backdrop-blur-sm">
      ...
      <div className="text-left sm:text-right z-10">
        ...
        <p className="text-slate-200 font-mono font-medium text-sm md:text-base">GST-2026-8X92B</p>
      </div>
    </div>
  );
};
    ...
    <div className="space-y-6 hidden lg:block">
      ...
      <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-xl p-5 shadow-xl">
        ...
        <div className="space-y-2 mb-3">
          ...
          <div className="flex items-center justify-between text-xs text-slate-300 bg-slate-800/40 p-2 rounded-lg border border-slate-700/30">
            <span>Email</span>
            <span className="text-sky-400 font-medium">help@gst.gov</span>
          </div>
        </div>
      </div>
...
export default function GstRegistrationForm({ user }: GstRegistrationFormProps) {
  ...
  const validateField = (name: keyof FormData, value: string): string => {
    switch (name) {
      ...
      case 'aadhaar':
        if (value && value.length > 0) return validators.aadhaar(value) === true ? '' : (validators.aadhaar(value) as string);
        return '';
      case 'zip':
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (key === 'aadhaar') formattedValue = value.replace(/\D/g, '').slice(0, 12);
  ...
  const getStepFields = (step: number): (keyof FormData)[] => {
    switch (step) {
      case 1:
        return ['legalName', 'pan', 'constitution'];
      case 2:
        return ['email', 'mobile', 'address1', 'city', 'zip', 'state'];
      default:
        return [];
    }
  };
    ...
    try {
      ...
      await displayRazorpay(0, async (response) => {
        ...
        await mockDbService.createDocument({
          ...
          serviceId: generateServiceId('GST'),
          status: 'paid',
        ...
        <div className="p-6 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div><span className="text-slate-500">Legal Name:</span> <span className="text-white">{formData.legalName || '—'}</span></div>
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-2xl shadow-2xl max-w-md w-full text-center border border-slate-700/50">
          ...
          <p className="text-slate-300 mb-8">
            ...
            <span className="text-sky-400 font-mono">GST-2026-8X92B</span>.
          </p>
          <button
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#020c1b] to-[#0a192f] p-4 sm:p-6 md:p-8">
      <div className="max-w-[1600px] mx-auto">
        <div className="lg:hidden mb-6 text-center">
          <h1 className="text-2xl font-bold text-white drop-shadow-lg">GST Registration</h1>
          ...
          <main className="lg:col-span-7 xl:col-span-8 glass-panel rounded-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] overflow-hidden relative min-h-[600px] flex flex-col">
            <div className="p-6 md:p-10 flex-grow">
              <div className="text-center mb-8 hidden lg:block">
                <h1 className="text-3xl font-bold text-white mb-3 tracking-tight drop-shadow-md">GST Registration</h1>
